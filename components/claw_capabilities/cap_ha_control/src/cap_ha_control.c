        ...
        .input_schema_json =
            ...
            "\"properties\":{"
              ...
              "\"trigger\":{\"type\":\"object\",\"properties\":{"
                ...
                "\"to\":{\"type\":\"string\",\"description\":\"state: target state. binary_sensor/light/switch/input_boolean use 'on'/'off' (door/window sensors display 'Open/Closed' in UI but the actual state value is on/off — firmware auto-normalizes 'open'->'on', 'closed'->'off'). cover uses 'open'/'closed'. lock uses 'locked'/'unlocked'.\"},"
                "\"from\":{\"type\":\"string\",\"description\":\"state: optional previous state (same vocabulary + same auto-normalization as 'to'). If omitted, firmware auto-fills the domain-pair opposite (binary_sensor/light/switch on<->off, cover open<->closed, lock locked<->unlocked) to force a HA transition. Specify explicitly to override.\"}"
              "}},"
              "\"condition\":{\"type\":\"object\",\"description\":\"Optional gate that must be true at trigger time. Single condition object — AND logic; multi-condition is not yet supported.\",\"properties\":{"
                ...
                "\"state\":{\"type\":\"string\",\"description\":\"state: required current state value (e.g., 'off' to fire only when the entity is off). Same vocabulary + auto-normalization as trigger.to — binary_sensor uses 'on'/'off' (firmware translates 'open'->'on', 'closed'->'off').\"}"
              "}},"
              "\"target\":{\"type\":\"string\",\"description\":\"HA entity friendly name or entity_id. board:* targets are not supported here (HA-side automation only).\"},"
...
void cap_ha_compose_description(void)
{
    ...
    snprintf(s_ha_automation_description, sizeof(s_ha_automation_description),
             ...
             "IMPORTANT: binary_sensor (door/window/motion) uses 'on'/'off' values, NOT 'open'/'closed' (firmware auto-normalizes if you forget — but prefer 'on'/'off' in tool calls). "
